...
pub var console: Video = undefined;
...
pub const Video = struct {
    ...
    height: u32,
    /// Row stride (pixels per scan line); may differ from `width`.
    pitch: u32,
    ...
    pub fn init(boot_info: *const FirmwareInterface) !Video {
        ...
        // GOP hands the kernel either RGBX or BGRX; the bootloader records the
        // actual mode in `framebuffer_pixel_format` (0 = RGBX, 1 = BGRX).
        const fmt = if (boot_info.framebuffer_pixel_format == 1)
            PixelFormat.BGRA
        else
            PixelFormat.RGBA;
        ...
        return Video{
            .framebuffer = @as(
                [*]volatile u32,
                @ptrFromInt(boot_info.framebuffer_base),
            ),
    ...
    fn scrollUp(self: *Video) void {
        ...
        const row_bytes = self.pitch * bytes_per_pixel;
